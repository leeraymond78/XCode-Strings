...
NSString * const kRegularExpressionPattern = @"^(\"(\\S+.*)\"|([^/]\\S+.*\\S+))\\s*=\\s*\"(.*)\";$";
...
+ (NSString*)_settingDirectory {
    ...
    NSString* settingDirectory = [(NSString*)[paths objectAtIndex:0] stringByAppendingPathComponent:@"XCodeStrings"];
...
+ (NSString*)recentFilePath {
    NSString* settingDirectory = [StringModel _settingDirectory];
    return [settingDirectory stringByAppendingPathComponent:@"XCodeStrings_recent.plist"];
}
...
- (instancetype)initWithType:(ActionType)type key:(NSString *)key identifier:(NSString*)identifier value:(NSString*)value
{
    self = [super init];
    if (self) {
